 ...
 */
 /*
 Project 4 main changes
 1. update statusId in disk table
 2. created view View_Borrower_No_Loans_Join
 3. created view View_Borrower_No_Loans_SubQ
 */
 --Create a database
USE master;
GO
...
UPDATE d
	SET d.statusId = 2
FROM disk d
INNER JOIN borrowedDisk bd
	ON d.diskId = bd.diskId
WHERE bd.returnDate IS NULL;

-- BEGIN Project 4.

-- 1. Show all disks in your database, the type, & status.
SELECT diskName, t.typeDescription, g.genreDescription, s.statusDescription
FROM disk d
INNER JOIN diskType t
	ON d.typeId = t.typeId
INNER JOIN genre g
	ON d.genreId = g.genreId
INNER JOIN diskStatus s
	ON d.statusId = s.statusId;

-- 2. Show all borrowed disks and who borrowed them.
SELECT b.LastName, b.firstName, d.diskName, bd.borrowedDate, bd.returnDate
FROM borrowedDisk bd
INNER JOIN borrower b
	ON bd.borrowerId = b.borrowerId
INNER JOIN disk d
	ON bd.diskId = d.diskId;

-- 3. Show the disks that have been borrowed more than once.
SELECT d.diskName, COUNT(bd.diskId) AS BorrowerCount
FROM disk d
INNER JOIN borrowedDisk bd
	ON d.diskId = bd.diskId
GROUP BY d.diskName
HAVING COUNT(bd.diskId) > 1;

-- 4. Show the disks outstanding or on-loan and who took each disk.
SELECT d.diskName, bd.borrowedDate, bd.returnDate, b.lastName, b.firstName
FROM disk d
INNER JOIN borrowedDisk bd
	ON d.diskId = bd.diskId
INNER JOIN borrower b
...
WHERE bd.returnDate IS NULL;
GO
-- 5. Create a view called View_Borrower_No_Loans_Join that shows the borrowers who have not borrowed a disk. 
--    Write as a join. Include the borrower id in the view definition. 
...
-- 6. Create a view called View_Borrower_No_Loans_SubQ that shows the borrowers who have not borrowed a disk. 
--    Write as a subquery. Include the borrower id in the view definition. 
CREATE VIEW View_Borrower_No_Loans_SubQ AS
SELECT b.lastName, b.firstName
FROM borrowedDisk bd
FULL OUTER JOIN borrower b
	ON bd.borrowerId = b.borrowerId
WHERE bd.borrowedDiskId IS NULL;
GO

-- 7. Show the borrowers who have borrowed more than 1 disk. 
SELECT b.lastName, b.firstName, COUNT(bd.diskId) AS diskBorrowed 
FROM borrowedDisk bd
INNER JOIN borrower b
	ON bd.borrowerId = b.borrowerId
